...
describe("MerkleTree", function () {
    ...
    it("Insert two new leaves and verify the first leaf in an inclusion proof", async function () {
        ...
        const { proof2, publicSignals2 } = await groth16.fullProve(Input, "circuits/circuit_js/circuit.wasm","circuits/circuit_final.zkey");

        const editedPublicSignals2 = unstringifyBigInts(publicSignals);
        const editedProof2 = unstringifyBigInts(proof);
        const calldata2 = await groth16.exportSolidityCallData(editedProof, editedPublicSignals);
