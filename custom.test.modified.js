...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    // withdraws 0.08 Ether from the shielded pool
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    // Alice deposits 0.13 Ether into tornado pool
    ...
    const postTransferAliceChangeUtxo = new Utxo({
    ...
    await transaction({
      tornadoPool,
      inputs: [aliceDepositUtxo],
      outputs: [bobSendUtxo, postTransferAliceChangeUtxo]})
    ...
    // withdraws 0.07 Ether from the shielded pool
    const aliceWithdrawAmount = postTransferAliceChangeUtxo.amount
    const recipient = '0x44AC194359fA44eCe6Cb2E53E8c90547BCCb95a0' // taking a random account for the test
    const aliceFinalChangeUtxo = new Utxo({
      amount: 0,
      keypair: aliceKeypair,
    })
    ...
    await transaction({
      tornadoPool,
      inputs: [postTransferAliceChangeUtxo],
      outputs: [aliceFinalChangeUtxo],
      recipient: recipient,
      isL1Withdrawal: true,
    })
